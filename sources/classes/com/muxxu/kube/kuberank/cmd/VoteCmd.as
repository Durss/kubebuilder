package com.muxxu.kube.kuberank.cmd {
	...
	public class VoteCmd extends LoadFileCmd implements Command {
		 ...
		 * Constructor
		 */
		public function  VoteCmd(cubeData:CubeData) {
			_cubeData = cubeData;
			super(Config.getPath("postVote"));
		...
		override protected function loadCompleteHandler(event:Event = null):void {
			...
			try {
				...
				throw new KubeException(Label.getLabel("errorVoteResultFormat"), KubeExceptionLevel.ERROR);
			}
			...
			if(result == "0") {
				...
				dispatchEvent(new CommandEvent(CommandEvent.ERROR));
				throw new KubeException(Label.getLabel("errorVoteResult"+result), KubeExceptionLevel.ERROR);
			}
		}
