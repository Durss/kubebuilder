package com.muxxu.kube.kuberank.cmd {
	 ...
	 * @date 24 avr. 2011;
	 */
	public class GetListsCmd extends LoadFileCmd implements Command {
		

		/**
		 * Constructor
		 */
		public function GetListsCmd() {
			super(Config.getPath("getLists"));
		}
		...
		override protected function loadCompleteHandler(event:Event = null):void {
			...
			try {
				...
				throw new KubeException(Label.getLabel("errorGetListsResultFormat"), KubeExceptionLevel.ERROR);
			}
			...
			if(result == "0") {
				...
				dispatchEvent(new CommandEvent(CommandEvent.ERROR));
				throw new KubeException(Label.getLabel("errorGetListsResult"+result), KubeExceptionLevel.ERROR);
			}
		}
		...
		override protected function loadErrorHandler(event:IOErrorEvent):void {
			...
			throw new KubeException(Label.getLabel("errorGetListsResult404"), KubeExceptionLevel.ERROR);
		}
